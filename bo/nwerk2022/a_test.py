...
def delay(user_pw: str, correct_pw: str) -> int:
    assert len(user_pw) > 0 and len(user_pw) < 21, (
        "Entered Password has wrong length (0<pw<21)"
    )
    assert len(correct_pw) > 0 and len(correct_pw) < 21, (
        "Correct Password has wrong length (0<pw<21)"
    )
...
parser = argparse.ArgumentParser(
    description="Testing tool for the Access Denied problem"
)
...
with subprocess.Popen(
    ...
    try:
        while True:
            guess = read(p)
            assert len(guess) > 0 and len(guess) < 21, (
                "Password has wrong length (0<pw<21)"
            )
            steps += 1
